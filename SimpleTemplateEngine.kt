 ...
 */
class SimpleTemplateEngine(private val root: File) {
	...
	private fun renderInternal(
		...
		val file = if (templatePath.startsWith('/')) {
			root.resolve(templatePath.substring(1))
		} else {
			root.resolve(templatePath)
		}
		val absPath = file.absoluteFile.normalize().path
		if (!file.exists() || !file.isFile) throw TemplateNotFound("Template not found: $templatePath (resolved: $absPath)")

		if (!seen.add(absPath)) throw IncludeCycleException("Include cycle detected: $absPath")
			...
			is String -> {
				...
				if (ref != null) {
					...
					if (resolved == null) {
						...
						JSONObject()
					} else if (resolved is JSONObject) resolved else JSONObject(mapOf("value" to resolved))
